...
- **Use Templates in Regular Creation** if enabled, will use templates for new objects when using New Card/Tube, Curve Card/Tube and Braid. By default it is disabled and only the Template button uses templates.
...
- **Colorize Regrouped Layers** will enable automatic colorization of the regrouped layers when pressing the **Regroup by Layer** button. You can change the colors in the :ref:`Curve Control Window<curve-control-window>` or using the :ref:`Custom Colors Window<layer-customization-window>`.
...
- **Ignore Last Layer** will change the behavior of the Extract All button and Filter buttons. Those buttons will either ignore or affect the last Layer depending on this option.
...
- **Number of Active Layers** allows the user to choose the number of active layers in the scene (up to 80).
...
- **Import Into a New Collection** will enable :ref:`Importing<import-export-page>` curves into a separate "Imported Curves" collection. Only works if **Show Layer Collections Menu** is checked.
- **Enable Mouse Scroll in Collections Dropdown** will allow scrolling through the collections list using the mouse wheel in the :ref:`Curve Control Window<curve-control-window>`. This will change the currently selected curves to the scrolled collection.
