...
Geometry and Curves generated by GS CurveTools plug-in are grouped together and only the pathCurve component is selectable in the Viewport by default. You can, however, switch between the components using Selection Functions.
...
Component Selection commands are multi-selection capable, so they will work on any number of selections.
...
It is recommended to use :ref:`Layers<layers>` to organize your curves. However, you can also quickly group selected curves (and their components) in the Maya Outliner using the **Group Curves** function.

**Group Curves** will group all the selected curves (and their components) in the Outliner and name the Group based on the Group Name input field. If the Input Field is empty, the group will have the default name (**crvGrp**).
...
**Regroup by Layer** function will take all the curves and group them in the outliner based on their Layer number and the name of the layer in the :ref:`Layer Customization Window<layer-customization-window>`.
...
- Holding the **Alt** button will deselect any selected CVs based on the slider position.
...
**Reset Pivot** button will simply reset the pivot point of any number of selected curves back to its original position at the Root CVs of the curve.

Holding the **Shift** modifier will position the pivot point at the last CV of the curve (tip of the curve).
